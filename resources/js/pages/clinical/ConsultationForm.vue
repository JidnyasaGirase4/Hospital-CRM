...
async function submit() {
    ...
    saving.value = true;
    try {
        await apiClient.post('/consultations', form.value);
        router.push({ name: 'consultations.index' });
    } catch (e) {
...
<template>
    <form class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 space-y-4" @submit.prevent="submit">
        <h2 class="text-lg font-semibold text-slate-800">New Consultation</h2>
        ...
        <div>
            ...
            <input v-model="form.chief_complaint" class="w-full border border-slate-300 rounded px-3 py-2 text-sm" />
        </div>
        <div>
        ...
        <div>
            <label class="block text-sm font-medium text-slate-700 mb-1">Examination</label>
            <textarea v-model="form.examination" rows="2" class="w-full border border-slate-300 rounded px-3 py-2 text-sm"></textarea>
        </div>
        <div>
        ...
        <div>
            ...
            <textarea v-model="form.clinical_notes" rows="2" class="w-full border border-slate-300 rounded px-3 py-2 text-sm"></textarea>
        </div>
        <div>
        ...
        <div class="flex justify-end gap-3">
            <RouterLink :to="{ name: 'consultations.index' }" class="px-4 py-2 text-sm text-slate-600 hover:text-slate-900">Cancel</RouterLink>
