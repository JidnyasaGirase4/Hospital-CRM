...
<template>
    <form class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 space-y-4" @submit.prevent="submit">
        <h2 class="text-lg font-semibold text-slate-800">New Radiology Order</h2>
        ...
        <div>
            <label class="block text-sm font-medium text-slate-700 mb-1">Test</label>
            <select v-model="form.radiology_test_id" class="w-full border border-slate-300 rounded px-3 py-2 text-sm">
                <option value="">Select…</option>
                <option v-for="t in tests" :key="t.id" :value="t.id">{{ t.name }} ({{ t.modality }})</option>
            </select>
            <p v-if="errors.radiology_test_id" class="text-xs text-red-600 mt-1">{{ errors.radiology_test_id[0] }}</p>
        </div>
        <div>
        ...
        <div class="flex justify-end gap-3">
            <RouterLink :to="{ name: 'radiology-orders.index' }" class="px-4 py-2 text-sm text-slate-600 hover:text-slate-900">Cancel</RouterLink>
